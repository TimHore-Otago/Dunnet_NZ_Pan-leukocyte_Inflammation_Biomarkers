...
#Inputting data 
input.files <- list.files(pattern = "MAP4K1", full.names = FALSE, recursive = FALSE)
input.files
...
for (i in 1:length(input.files)) {
  ...
  MethylationCat <- function(MethCat) {
    if (MethCat >= 4/6) { return("Leukocyte") }
    if (MethCat > 2/5) {return("Unknown") }
    else if (MethCat <= 2/5) {return("Non-Leukocyte")}
  }
...
write.table(CombinedCallData, paste("MAP4K1_Combined_read_call.txt", sep = ""), 
